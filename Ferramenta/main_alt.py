...
def load_config():
    # TODO: transformar nome do arquivo em env var
    # Se forem vários arquivos, retornar como lista? Ou vai rodar uma instância por "configuração"?
    config_filename = "configs/config.yaml"
    with open(config_filename) as config_file:
        try:
            return yaml.safe_load(config_file)
        except Exception as e:
            logging.debug(f"Falha ao carregar arquivo de configuração {config_filename}")
...
def load_arcgis_variables(variables_obj):
    ...
    if variables_obj.arcgis_execution:
        ...
        if config != None and config['target_area'] != '':
        ...
        if config != None and config['max_cloud_coverage'] != '':
        ...
        if config != None and config['classification_arguments'] != '':
            classification_arguments = config['classification_arguments']
        ...
        if config != None and config['ml_model'] != '':
        ...
        if config != None and config['max_date'] != '':
        ...
        if config != None and config['days_period'] != '':
            days_period = config['days_period']
        if days_period:
...
def get_images():
    ...
    images.get_historic_and_current_images(
        ...
        compose_as_single_image=VARIABLES.compose_as_single_image,
        # Caso negativo, os tiles não serão mosaicados em uma única imagem, isto impossibilita o append em um Mosaic Dataset
...
def add_image_to_mosaic_dataset(image: Image, location: Database, mosaic_dataset: MosaicDataset,
                                satelite: str = 'Sentinel2'):
